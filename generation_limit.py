...
class TrajectoryGenerator:
    ...
    def compare_trajectories(self, target_df, user_df, classification_result, generation_number=1):
        ...
        # ─────────────────────────────────────────────────────
        # [추가] 각도 범위 제한 (Clamping)
        # deg1 : -10 ~ 110
        # deg2 :  0  ~ 150
        # deg3 :  0  ~ 150
        # deg4 : -90 ~  90
        # ─────────────────────────────────────────────────────
        joint_mins = np.array([-10,   0,   0, -90], dtype=float)
        joint_maxs = np.array([110, 150, 150,  90], dtype=float)
        aligned_degrees = np.clip(aligned_degrees, joint_mins, joint_maxs)
